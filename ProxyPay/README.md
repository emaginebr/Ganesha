# ProxyPay - Client SDK for the ProxyPay Sales Platform API
...
**ProxyPay** is a .NET 8 client SDK (Anti-Corruption Layer) that provides typed HTTP clients and DTOs for consuming the ProxyPay Sales Platform REST API. It allows external applications to integrate with the ProxyPay backend for managing **stores**, **products**, **categories**, **images**, **store users**, and **shopping carts** without coupling to internal API details.

Part of the **ProxyPay ecosystem** — this package is consumed by frontend applications and other services that need to interact with the ProxyPay API.
...
│   │   └── ProductTypeEnum.cs
│   ├── Settings/
│   │   └── ProxyPaySetting.cs     # API URL + Bucket configuration
│   ├── ShopCar/
│   │   └── ShopCarInfo.cs
│   └── Store/
│       ├── StoreInfo.cs
...
|---------|------|-------------|
| **ProxyPay.API** | Backend | REST API + GraphQL endpoints |
| **ProxyPay.Domain** | Backend | Business logic and domain services |
| **ProxyPay.Infra** | Backend | EF Core repositories and database context |
| **ProxyPay** | NuGet Package | Client SDK (this package) |
...
```bash
dotnet add package ProxyPay
```

### Build from Source

```bash
dotnet build ProxyPay.csproj
```
...
Add the ProxyPay settings to your `appsettings.json`:

```json
{
  "ProxyPaySetting": {
    "ApiUrl": "https://your-proxypay-api-url",
...
```csharp
services.Configure<ProxyPaySetting>(configuration.GetSection("ProxyPaySetting"));
